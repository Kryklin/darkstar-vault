...
describe('CryptService - Authenticated Streaming & Legacy Container Security', () => {
  ...
  it('should reject outer totalSize tampering in v2 container', async () => {
    ...
    await expectAsync(service.decryptBinary(tamperedPayload, 'test-key')).toBeRejectedWithError(
      /Streaming integrity violation.*tampering detected/
    );
  });
  ...
  it('should reject streamId tampering in v2 container', async () => {
    ...
    await expectAsync(service.decryptBinary(tamperedPayload, 'test-key')).toBeRejectedWithError(
      /Streaming integrity violation.*metadata tampering detected/
    );
  });
  ...
  it('should reject chunk deletion tampering in v2 container', async () => {
    ...
    await expectAsync(service.decryptBinary(tamperedPayload2, 'test-key')).toBeRejectedWithError(
      /Streaming integrity violation.*metadata tampering detected/
    );
  });
  ...
  it('should reject non-container raw data without falling through to single-blob decryption', async () => {
    ...
    await expectAsync(service.decryptBinary(rawData, 'test-key')).toBeRejectedWithError(
      /Streaming integrity violation: payload is not a valid container JSON/
    );
  });
